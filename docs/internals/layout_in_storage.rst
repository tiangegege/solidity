...
:ref:`rules above <storage-inplace-encoding>` and the actual data of mappings and
dynamcically-sized arrays is stored starting at a different
...
different hash distribution to store their data due to a different starting slot.
...
The slot is ``keccak256(keccak256(p) + i) + floor(j / 8)`` (8 items per slot because 256/32 = 8) and
the element can be obtained from the slot data ``v`` using ``(v >> ((j % 8) * 32)) & 0xffffffff``.
...
The type of the value is ``uint256``, so it uses the full slot.
