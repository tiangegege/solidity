...
MemberList::MemberMap AddressType::nativeMembers(ASTNode const*) const
{
	...
	if (m_stateMutability == StateMutability::Payable)
	{
		members.emplace_back(MemberList::Member::native("send", TypeProvider::function(strings{"uint"}, strings{"bool"}, FunctionType::Kind::Send, false, StateMutability::NonPayable)));
		members.emplace_back(MemberList::Member::native("transfer", TypeProvider::function(strings{"uint"}, strings(), FunctionType::Kind::Transfer, false, StateMutability::NonPayable)));
	}
	return members;
}
...
{
	return MemberList::MemberMap{MemberList::Member::native("length", TypeProvider::uint(8))};
}
